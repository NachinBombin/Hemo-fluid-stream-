hook.Add("PopulateToolMenu", "OFSimpleBleeding_PopulateMenu", function()
    spawnmenu.AddToolMenuOption("Utilities", "Admin", "OFSimpleBleedingSettings", "Juicy Bleeding", "", "", function(panel)
        panel:ClearControls()
        
        panel:CheckBox("Enable Bleeding Effect", "of_bleeding_enabled")
        panel:CheckBox("Enable Bleeding for Players", "of_bleeding_player"):SetTooltip("Allow players to show blood effects when damaged")
        panel:NumSlider("Maximum Active Blood Effects", "of_bleeding_maxactive", 10, 500, 0)
        panel:NumSlider("Bleeding Cooldown Time", "of_bleeding_cooldown", 0, 1, 1)
        panel:CheckBox("Debug Mode", "of_bleeding_debug")
        panel:CheckBox("Use Darker Blood Color", "of_bleeding_darker")
...
hook.Add("PostDrawTranslucentRenderables", "OFSimpleBleeding_DebugHiddenModel", function(depth, skybox, skybox3d)
    if skybox or skybox3d then return end

    local cvar = GetConVar("of_bleeding_debug")
    if not cvar or not cvar:GetBool() then return end

    for _, ent in ipairs(ents.GetAll()) do
        if not IsValid(ent) then continue end
        if not ent:GetNW2Bool("of_bleeding_debug", false) then continue end

        local pos = ent:GetPos()
        local ang = ent:GetAngles()

        -- 根据type选择球的颜色
        local debug_type = ent:GetNW2Int("of_bleeding_debug_type", 0)
        ...
        -- 显示骨骼名字
        local bone_name = ent:GetNW2String("of_bleeding_debug_bone_name", "unknown")
